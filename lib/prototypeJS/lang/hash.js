  ...
  **/
  ...
  function diff(object) {
    ...
    object.keys().forEach(function(key){
      var before = self._object[key]
      var after = object._object[key]
      if (before != after){
        if (Object.isArray(after)){
          if (!Object.isArray(before)) before = []
          result[key] = after.diff(before, compare)
        } else {
          result[key] = after
        }
        diff = true;
  ...
  function diffMerge(diff){
    ...
    $H(diff).keys().forEach(function(key){
      ...
      if (before != after){
        if (Object.isArray(after)){
          if (!Object.isArray(before)) before = []
          self._object[key] = before.diffMerge(after, compare)
        } else {
          self._object[key] = after
        }
      }
    })
    return self
